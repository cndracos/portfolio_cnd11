...
1. The ball 'chews up' block when hitting a corner from the side, can go very slowly if hits the corner at a small angle.
2. If the ball is caught between the paddle and the side of the screen, the ball will get stuck in the side of the screen, with no way to end the level. 
3. The multiple poorly designed data dependencies between classes, with main culprits being the other paddle, SIZE int, and brickHits int.